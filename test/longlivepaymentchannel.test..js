...
contract(
  ...
  (accounts) => {
    ...
    it("the recipient should be able to withdraw from the channel", async () => {
      ...
      const recBalanceShouldBe = web3.utils
      ...
      assert.equal(
        recBalanceShouldBe,
        recBalanceAfter,
        `Recipient Balance Should be ${recBalanceShouldBe} but is ${recBalanceAfter}`
      );
    });
    ...
    it("should able to close the payment channel", async () => {
      ...
      assert.equal(
        ...
        `Recipient balance shoud be ${recBalanceShouldBe}, but is ${recipientBalanceAfter}`
      );
